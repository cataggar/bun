...
const State = struct {
                    ...
                    .exited => |exited| {
                        if (exited.code == 0) {
                            if (handle.start_time != null and handle.end_time != null) {
                                const duration = handle.start_time.?.durationTo(handle.end_time.?);
                                const ms = @as(f64, @floatFromInt(duration.toNanoseconds())) / 1_000_000.0;
                                if (ms > 1000.0) {
    ...
    fn flushDrawBuf(this: *This) void {
        std.Io.File.stdout().writeStreamingAll(bun.blockingIo(), this.draw_buf.items) catch {};
    }
