...
pub const PatchFile = struct {
    ...
    pub fn apply(this: *const PatchFile, allocator: Allocator, patch_dir: bun.FD) ?bun.sys.Error {
        ...
        for (this.parts.items) |*part| {
            ...
            switch (part.*) {
                .file_deletion => {
                    const pathz = bun.handleOom(bun.dupeZ(bun, arena.allocator(), u8, part.file_deletion.path));
                ...
                .file_rename => {
                    const from_path = bun.handleOom(bun.dupeZ(bun, arena.allocator(), u8, part.file_rename.from_path));
                    const to_path = bun.handleOom(bun.dupeZ(bun, arena.allocator(), u8, part.file_rename.to_path));
                ...
                .file_creation => {
                    const filepath = bun.PathString.init(bun.handleOom(bun.dupeZ(bun, arena.allocator(), u8, part.file_creation.path)));
                ...
                .file_mode_change => {
                    ...
                    const filepath = bun.handleOom(bun.dupeZ(bun, arena.allocator(), u8, part.file_mode_change.path));
    ...
    fn applyPatch(
        ...
        const file_path: [:0]const u8 = bun.handleOom(bun.dupeZ(bun, arena.allocator(), u8, patch.path));
...
pub fn gitDiffPreprocessPaths(
    ...
    if (bun.Environment.isPosix and sentinel) {
        return .{
            bun.handleOom(bun.dupeZ(bun, allocator, u8, old_folder)),
            bun.handleOom(bun.dupeZ(bun, allocator, u8, new_folder)),
        };
    }
