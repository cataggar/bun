...
pub fn waitForDebuggerIfNecessary(this: *VirtualMachine) void {
    ...
    if (comptime Environment.enable_logs)
        Debugger.log("waitForDebugger: {f}", .{Output.ElapsedFormatter{
            ...
            .duration_ns = @truncate(@as(u128, @intCast(bun.SystemTimer.nanoTimestamp() - bun.cli.start_time))),
        }});
    ...
    while (debugger.wait_for_connection != .off) {
        ...
        switch (debugger.wait_for_connection) {
            .forever => {
                ...
                if (comptime Environment.enable_logs)
                    log("waited: {f}", .{bun.fmt.fmtDurationOneDecimal(@intCast(@as(i64, @truncate(bun.SystemTimer.nanoTimestamp() - bun.cli.start_time))))});
            },
            .shortly => {
