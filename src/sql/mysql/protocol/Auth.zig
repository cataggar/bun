...
pub const mysql_native_password = struct {
    pub fn scramble(password: []const u8, nonce: []const u8) ![20]u8 {
        ...
        var stage1 = @as([20]u8, @splat(0));
        var stage2 = @as([20]u8, @splat(0));
        var stage3 = @as([20]u8, @splat(0));
        var result: [20]u8 = @as([20]u8, @splat(0));
        if (password.len == 0) {
            return result;
        }
...
pub const caching_sha2_password = struct {
    pub fn scramble(password: []const u8, nonce: []const u8) ![32]u8 {
        ...
        var digest1 = @as([32]u8, @splat(0));
        var digest2 = @as([32]u8, @splat(0));
        var digest3 = @as([32]u8, @splat(0));
