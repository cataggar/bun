...
pub fn ComptimeClap(
    ...
    return struct {
        ...
        pub fn parse(iter: anytype, opt: clap.ParseOptions) !@This() {
            ...
            var multis = @as([multi_options]std.array_list.Managed([]const u8), @splat(undefined));
            for (&multis) |*multi| {
            ...
            var res = @This(){
                .single_options = @as([single_options]?[]const u8, @splat(null)),
                .multi_options = @as([multi_options][]const []const u8, @splat(undefined)),
                .flags = @as([flags]bool, @splat(false)),
                .pos = undefined,
                .allocator = allocator,
