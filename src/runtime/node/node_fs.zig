...
pub const AsyncReaddirRecursiveTask = struct {
    ...
    pub fn enqueue(
        ...
        var task = Subtask.new(
            .{
                ...
                .basename = bun.PathString.init(bun.handleOom(bun.dupeZ(bun, bun.default_allocator, u8, basename))),
            },
        );
    ...
    pub fn create(
        ...
        var task = AsyncReaddirRecursiveTask.new(.{
            ...
            .tracker = jsc.Debugger.AsyncTaskTracker.init(vm),
            .subtask_count = .{ .raw = 1 },
            .root_path = PathString.init(bun.handleOom(bun.dupeZ(bun, bun.default_allocator, u8, args.path.slice()))),
            ...
            while (switch (entry) {
                ...
                enqueue: {
                    switch (current.kind) {
                        ...
                        => {
                            if (current.name.len + 1 + name_to_copy.len > bun.MAX_PATH_BYTES) break :enqueue;
                            stack.writeItem(bun.dupeZ(bun, basename_allocator, u8, name_to_copy) catch break :enqueue) catch break :enqueue;
                        },
                        // Some filesystems (e.g., Docker bind mounts, FUSE, NFS) return
                        // DT_UNKNOWN for d_type. Use lstatat to determine the actual type.
                        .unknown => {
                ...
                if (bun.StandaloneModuleGraph.get()) |graph| {
                    if (graph.find(path)) |file| {
                        if (args.encoding == .buffer) {
                            ...
                            return .{
                                .result = .{
                                    .null_terminated = bun.handleOom(bun.dupeZ(bun, bun.default_allocator, u8, file.contents)),
                                },
                            };
                    }
                }
                ...
                switch (args.encoding) {
                    .buffer => {
                        ...
                        if (comptime string_type == .default) {
                            ...
                            return .{
                                .result = .{
                                    .null_terminated = bun.dupeZ(bun, bun.default_allocator, u8, temporary_read_buffer) catch return .{
                                        .err = Syscall.Error.fromCode(.NOMEM, .read).withPathLike(args.path),
                                    },
                                },
                            };
