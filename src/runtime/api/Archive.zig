...
fn buildTarballFromObject(globalThis: *jsc.JSGlobalObject, obj: jsc.JSValue) bun.JSError![]u8 {
    ...
    const now_millis = std.Io.Clock.real.now(bun.blockingIo()).toMilliseconds();
    const now_secs: isize = @intCast(@divTrunc(now_millis, 1000));
    ...
    while (try iter.next()) |key| {
        ...
        const key_str = try bun.dupeZ(bun, allocator, u8, key_slice.slice());
...
fn startWriteTask(
    ...
    const path_z = try bun.dupeZ(bun, bun.default_allocator, u8, path);
...
const FilesContext = struct {
    ...
    fn cloneErrorString(archive: *libarchive.lib.Archive) ?[*:0]u8 {
        ...
        return bun.dupeZ(bun, bun.default_allocator, u8, err_str) catch null;
    }
