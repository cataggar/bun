...
pub const CronRegisterJob = struct {
    ...
    pub fn cronRegister(globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!jsc.JSValue {
        ...
        job.* = .{
            ...
            .bun_exe = bun_exe,
            .abs_path = abs_path,
            .schedule = bun.handleOom(bun.dupeZ(bun, bun.default_allocator, u8, normalized_schedule)),
            .title = bun.handleOom(bun.dupeZ(bun, bun.default_allocator, u8, title_slice.slice())),
            .parsed_cron = parsed,
...
fn resolvePath(globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame, path: []const u8) ![:0]const u8 {
    ...
    return bun.dupeZ(bun, bun.default_allocator, u8, entry_path.text);
}
...
fn makeTempPath(comptime prefix: []const u8) ![:0]const u8 {
    ...
    return bun.dupeZ(bun, bun.default_allocator, u8, bun.path.joinAbsString(bun.fs.FileSystem.RealFS.platformTempDir(), &.{name}, .auto));
}
