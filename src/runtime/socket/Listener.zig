...
pub fn listen(globalObject: *jsc.JSGlobalObject, opts: JSValue) bun.JSError!JSValue {
    ...
    const listen_socket: *uws.ListenSocket = brk: {
        switch (connection) {
            .host => |host| {
                const hostz = bun.handleOom(bun.dupeZ(bun, bun.default_allocator, u8, host.host));
            ...
            .unix => |u| {
                const pathz = bun.handleOom(bun.dupeZ(bun, bun.default_allocator, u8, u));
...
pub fn addServerName(this: *Listener, global: *jsc.JSGlobalObject, hostname: JSValue, tls: JSValue) bun.JSError!JSValue {
    ...
    const server_name = bun.handleOom(bun.dupeZ(bun, bun.default_allocator, u8, host_str.slice()));
