        ...
        pub fn doConnect(this: *This, connection: Listener.UnixOrHost) !void {
            ...
            switch (connection) {
                .host => |host| {
                    ...
                    const hostz = bun.handleOom(bun.dupeZ(bun, alloc, u8, clean));
                ...
                .unix => |u| {
                    var sf = bun.stackFallback(1024, bun.default_allocator);
                    const alloc = sf.get();
                    const pathz = bun.handleOom(bun.dupeZ(bun, alloc, u8, u));
                ...
                if (this.socket.ssl()) |ssl_ptr| {
                    if (!ssl_ptr.isInitFinished()) {
                        if (this.server_name) |server_name| {
                            ...
                            if (host.len > 0) {
                                const host__ = bun.handleOom(bun.dupeZ(bun, default_allocator, u8, host));
