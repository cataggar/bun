...
fn findChrome(alloc: std.mem.Allocator, explicitPath: ?[*:0]const u8) !?[:0]const u8 {
    ...
    if (explicitPath) |p| {
        return try bun.dupeZ(bun, alloc, u8, std.mem.span(p));
    }
    if (bun.getenvZ("BUN_CHROME_PATH")) |p| {
        return try bun.dupeZ(bun, alloc, u8, p);
    }
    ...
    for (names) |n| {
        if (bun.which(buf, path, "", n)) |found| {
            return try bun.dupeZ(bun, alloc, u8, found);
        }
    }
    ...
    if (comptime bun.Environment.isMac) {
        ...
        for (bundles) |b| {
            ...
            if (bun.sys.isExecutableFilePath(sys)) return try bun.dupeZ(bun, alloc, u8, sys);
            if (home.len > 0) {
                const user_parts = [_][]const u8{ home, "Applications", b };
                const user = bun.path.joinStringBufZ(buf, &user_parts, .auto);
                if (bun.sys.isExecutableFilePath(user)) return try bun.dupeZ(bun, alloc, u8, user);
            }
        }
        ...
        for (absolute) |c| {
            if (bun.sys.isExecutableFilePath(c)) return try bun.dupeZ(bun, alloc, u8, c);
        }
    }
...
fn findPlaywrightShell(alloc: std.mem.Allocator) ?[:0]const u8 {
    ...
    if (bun.sys.isExecutableFilePath(bin)) return bun.dupeZ(bun, alloc, u8, bin) catch return null;
    ...
    if (comptime bun.Environment.isLinux and bun.Environment.isAarch64) {
        ...
        if (bun.sys.isExecutableFilePath(bin2)) return bun.dupeZ(bun, alloc, u8, bin2) catch return null;
    }
    return null;
}
